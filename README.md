...
2. Build and start the production stack:
	 ...
	 - Gunicorn runs Django app on port `127.0.0.1:8000` by default.
...
3. Put a host-level Nginx in front of the compose stack for TLS termination and public traffic. Certbot can add the HTTPS block automatically.
...
- `nginx/default.conf` — host Nginx config template
...
- See `.env.dev.example` and `.env.prod.example` for required variables:
	- `SECRET_KEY`, `DEBUG`, `ALLOWED_HOSTS`, `CSRF_TRUSTED_ORIGINS`, `WEB_PORT`
...
Use the host Nginx to terminate TLS, serve static/media from the bind-mounted directories, and forward app traffic to the compose stack at `127.0.0.1:8000`.

1. Point your domain DNS `A` record to the server IP.
2. Open firewall ports `80` and `443`.
3. Install Nginx and Certbot on the host.
4. Start the compose stack so the app is reachable on `127.0.0.1:8000`.
5. Configure host Nginx to serve static/media from the repo's `vol/static` and `vol/media` directories.
6. Configure host Nginx to proxy app requests to `http://127.0.0.1:8000`.
7. Run Certbot with the Nginx plugin so it can add the HTTPS server block automatically.
...
server {
    listen 80;
    server_name ladipoe.live www.ladipoe.live;
    ...
    location /static/ {
        alias /home/devops/poe/vol/static/;
        access_log off;
        expires 30d;
    }

    location /media/ {
        alias /home/devops/poe/vol/media/;
        access_log off;
        expires 30d;
    }
...
```sh
sudo certbot --nginx -d ladipoe.live -d www.ladipoe.live
```

After installation, verify that:

- the app responds at `https://ladipoe.live`
...
- Production uses Postgres in Docker and expects the host Nginx to proxy TLS traffic to `127.0.0.1:8000`.
